""" Récupérations des données et affichage des messages"""
...
def envoyerMessage(request):
    """ gestion envoi message depuis formulaire de message"""
    return render(request,'message/formMessage.html')


def addRequest(request):
    """ formulaire d'une création de demande"""
    ...
    return render(request, 'message/formRequest.html',
                  {'typeDemandes': objTypes, 'sendBy': user.service,
                   'employees': objEmployees, 'acces': objAcces,
                   'ordinateurs':objOrdinateur, 'telephones': objTelephone})


def saveRequest(request):
    """ Gestion d'un type de demande et récupération des données par référence """
    error = False
    if request.POST:
        data = request.POST
        if 'typeDemande' in data:
            ...
            # Variable unused: sendBy = data.get('sendBy')
        ...
        sendBy = "RH"
        if not error:
            newMessage = Message(typeDemande=typeDemande,
                                 ordinateur=refOrdi,
                                 telephone=refPhone,
                                 acces=refAcces,
...
def validRequest(request, id):
    """ Paramètres de validation d'une demande"""
    try:
...
def refuseRequest(request, id):
    """ Paramètres du refus de demande """
    try:
        # obj = Message.objects.filter(id=id)
        # Cette variable est considérée unused : obj = Message.objects.filter(id=id).update(is_active=False)
        user = request.session['user_id']
        user = Employee.objects.get(id=user)
        objTypes = TypeDemande.objects.all()
        objEmployees = Employee.objects.filter(refOrdi=None)
        ...
        objTelephone = Telephone.objects.all()
        return render(request, 'message/formRequest.html',
                      {'typeDemandes': objTypes,
                       'sendBy': user.service,
                       'employees': objEmployees,
                       'acces': objAcces,
                       'ordinateurs': objOrdinateur,
                       'telephones': objTelephone}
                      )
...
def logOut(request):
    """ Deconnexion du user et renvoie vers la mire de connexion"""
